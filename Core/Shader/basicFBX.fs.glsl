...
void main(void)
{
	vec2 texcoords = vec2(inFData.texCoord.x, 1.0 - inFData.texCoord.y);

	vec4 diffuseColor = texture(u_DiffuseTexture, texcoords);
    vec4 specularColor = texture(u_SpecularTexture, texcoords);

	vec4 FinalColor = vec4(0.0, 0.0, 0.0, 0.0);

    for (int idx = 0; idx < lightNb; idx++)
		FinalColor +=  ApplyLight(inLight[idx], diffuseColor) ;
