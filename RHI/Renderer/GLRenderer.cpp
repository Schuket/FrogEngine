...
namespace FrogEngine
{
	namespace OPENGL
	{
		...
		int feRenderer::Draw(size_t count, TypeDraw typeDraw, TypePolygon typePoly)
		{
			Error error;

			if (typeDraw == TypeDraw::DRAW)
			{
				error = RHIGLDrawArrays(ConvertTypePolygon(typePoly), 0, (GLsizei)count);
				if (error)	return error;
			}
			else if (typeDraw == TypeDraw::DRAW_ELEMENT)
			{
				error = RHIGLDrawElements(ConvertTypePolygon(typePoly), (GLsizei)count, GL_UNSIGNED_SHORT, nullptr);
				if (error)	return error;
			}
			else if (typeDraw == TypeDraw::DRAW_INDEXED)
			{
				error = RHIGLDrawElementBaseVertex(ConvertTypePolygon(typePoly), (GLsizei)count, GL_UNSIGNED_SHORT, nullptr, 0);
				if (error)	return error;
			}
		...
		GLenum feRenderer::ConvertTypePolygon(TypePolygon typePoly)
		{
			switch (typePoly)
			{
			case TypePolygon::POINTS:
				return GL_POINTS;
			case TypePolygon::LINES:
				return GL_LINES;
			case TypePolygon::PATCHES:
				return GL_PATCHES;
			case TypePolygon::TRIANGLES:
				return GL_TRIANGLES;
			default:
				return GL_NONE;
			}
		...
		Error feRenderer::RHIGLDrawElements(GLenum mode, GLint first, GLsizei count, GLvoid* indices)
		{
			...
			if (!error)
				...
				return RHIError::CastError(Error::RHI_INVALID_ENUM, WARNING, "feRenderer::RHIGLDrawElements : mode is not an accepted value.");
			else if (error == GL_INVALID_VALUE)
				return RHIError::CastError(Error::RHI_INVALID_VALUE, WARNING, "feRenderer::RHIGLDrawElements : count is negative.");
			else if (error == GL_INVALID_OPERATION)
				return RHIError::CastError(Error::RHI_INVALID_OPERATION, WARNING, "feRenderer::RHIGLDrawElements : a non-zero buffer object name is bound to an enabled array and the buffer object's _data store is currently mapped or a geometry shader is active and mode is incompatible with the input primitive type of the geometry shader in the currently installed program object.");

			return RHIError::CastError(RHI_UNKNOWN_ERROR, CRITICAL, "feRenderer::RHIGLDrawElements");
		}
